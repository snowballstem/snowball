...
static void generate_AE(struct generator * g, struct node * p) {
    ...
    switch (p->type) {
        ...
        case c_number:
            ...
            break;
        case c_maxint:
            write_string(g, "i32::MAX");
            break;
        case c_minint:
            write_string(g, "i32::MIN");
            break;
        case c_neg:
            write_char(g, '-');
        ...
        case c_cursor:
            ...
            break;
        case c_limit:
            w(g, p->mode == m_forward ? "env.limit" : "env.limit_backward");
            break;
        case c_len:
...
static void generate_stringassign(struct generator * g, struct node * p) {
    ...
    if (keep_c) {
        writef(g, "~Mlet c = env.cursor;~N", p);
    }
