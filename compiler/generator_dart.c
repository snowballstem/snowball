...
static void write_varref(struct generator * g, struct name * p) {
    /* In Dart, references look just the same */
    write_varname(g, p);
}
...
static void generate_AE(struct generator * g, struct node * p) {
    ...
    switch (p->type) {
        ...
        case c_number:
            ...
            break;
        case c_maxint:
            write_string(g, "9007199254740992");
            break;
        case c_minint:
            write_string(g, "-9007199254740992");
            break;
        ...
        case c_cursor:
            ...
            break;
        case c_limit:
            w(g, p->mode == m_forward ? "limit" : "limit_backward");
            break;
...
static void generate_and(struct generator * g, struct node * p) {
    ...
    if (K_needed_for_connective(p->left)) {
...
static void generate_assignto(struct generator * g, struct node * p) {
    write_comment(g, p);
    writef(g, "~M~V = assign_to();~N", p);
}

static void generate_sliceto(struct generator * g, struct node * p) {
    write_comment(g, p);
    writef(g, "~M~V = slice_to();~N", p);
}
...
static void generate_class_begin(struct generator * g) {
    w(g, "// ignore_for_file: non_constant_identifier_names, slash_for_doc_comments, camel_case_types~N~N");

    w(g, "import '../src/snowball.dart';~N~N");
...
static void generate_equals(struct generator * g) {
    w(g, "~N"
         "~M@override~N"
         "~Mbool operator ==(Object other) => other is ");
    write_s(g, g->options->name);
    w(g, ";~N~N"
         "~M@override~N"
         "~Mint get hashCode => runtimeType.toString().hashCode;~N");
}
...
static void generate_grouping_table(struct generator * g, struct grouping * q) {
    ...
    write_string(g, " = String.fromCharCodes([");
