...
//TODO: test->snowball pr
//TODO: pub.dev package
...
static void generate_AE(struct generator * g, struct node * p) {
    ...
    switch (p->type) {
        ...
        case c_maxint:
            write_string(g, "9007199254740991"); break;
        case c_minint:
            write_string(g, "-9007199254740991"); break;
        case c_neg:
...
static void generate_assignfrom(struct generator * g, struct node * p) {
    ...
    write_comment(g, p);
    if (keep_c) writef(g, "~{~Mvar c = cursor;~N", p);
    if (p->mode == m_forward) {
...
static void generate_setlimit(struct generator * g, struct node * p) {
    ...
    if (p->left && p->left->type == c_tomark) {
        ...
        if (p->mode == m_forward) {
            w(g, "~Mvar ~B0 = limit - cursor;~N");
            w(g, "~Mlimit = ");
        } else {
            w(g, "~Mvar ~B0 = limit_backward;~N");
            w(g, "~Mlimit_backward = ");
        }
...
static void generate_dollar(struct generator * g, struct node * p) {
    ...
    g->B[0] = str_data(savevar);
    writef(g, "~{~C~N"
              "~MBaseStemmer ~B0 = BaseStemmer();~N"
              "~M~B0.copy_from(this)~N", p);
...
static void generate_define(struct generator * g, struct node * p) {
    ...
    if (p->amongvar_needed) w(g, "~Mvar among_var;~N");
    str_clear(g->failure_str);
...
static void generate_class_begin(struct generator * g) {
    w(g, "import './../base_stemmer.dart';~N");
    if (g->analyser->among_count > 0) {
        w(g, "import './../among.dart';~N~N");
    }
...
static void generate_among_table(struct generator * g, struct among * x) {
    ...
    w(g, "~Mlate final List<Among> a_~I0 = [~N~+");
    {
        int i;
