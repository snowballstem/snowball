...
static void generate_AE(struct generator * g, struct node * p) {
    ...
    switch (p->type) {
        ...
        case c_number:
            ...
            break;
        case c_maxint:
            write_string(g, "PHP_INT_MAX");
            break;
        case c_minint:
            write_string(g, "PHP_INT_MIN");
            break;
        case c_neg:
            write_char(g, '-');
...
static void generate_or(struct generator * g, struct node * p) {
    ...
    while (p->right != NULL) {
        ...
        generate(g, p);
        if (!g->unreachable) {
            if (out_lab < 0) out_lab = new_label(g);
            wgotol(g, out_lab);
...
static void generate_stringassign(struct generator * g, struct node * p) {
    ...
    if (keep_c) {
        w(g, "~M$c = $this->cursor;~N");
        if (p->mode == m_forward) {
            writef(g, "~M$this->insert($c, $this->limit, ", p);
        } else {
            writef(g, "~M$this->insert($this->limit_backward, $c, ", p);
        }
    } else {
        if (p->mode == m_forward) {
...
static void generate_grouping(struct generator * g, struct node * p, int complement) {
    ...
    if (tailcallable(g, p)) {
        writef(g, "~Mreturn $this->~S1_grouping~S0(~V);~N", p);
        p->right = NULL;
    } else {
...
static void generate_class_begin(struct generator * g) {
    w(g, "class Snowball~n extends SnowballStemmer~N");
    write_block_start(g);
}
