...
static void generate_AE(struct generator * g, struct node * p) {
    ...
    switch (p->type) {
        ...
        case c_number:
            ...
            break;
        case c_maxint:
            write_string(g, "snowball.MaxInt");
            break;
        case c_minint:
            write_string(g, "snowball.MinInt");
            break;
        case c_neg:
            write_char(g, '-');
        ...
        label0:
            ...
            break;
        case c_cursor:
            w(g, "@as(i32, @intCast(env.cursor))");
            break;
        case c_limit:
            w(g, p->mode == m_forward ? "@as(i32, @intCast(env.limit))" : "@as(i32, @intCast(env.limit_backward))");
            break;
        case c_len:
...
static void generate_stringassign(struct generator * g, struct node * p) {
    ...
    if (keep_c) {
        writef(g, "~Mconst c = env.cursor;~N", p);
    }
    if (p->mode == m_forward) {
...
static void generate_slicefrom(struct generator * g, struct node * p) {
    ...
    if (p->literalstring && SIZE(p->literalstring) == 0) {
        w(g, "~Menv.sliceDel() catch return false;~N");
        return;
    }
    w(g, "~Menv.sliceFrom(");
    generate_address(g, p);
    w(g, ") catch return false;~N");
}
